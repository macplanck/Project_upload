    ...
    def sram_test(self, sram_token_in, sram_weight_in, sum_scale, max_scale):
        ...
        while range_token[0] < len_token:
            if range_token[1] > len_token:
                vec_receive = sram_token_in[ range_token[0] : len_token ]
            else:
                vec_receive = sram_token_in[ range_token[0] : range_token[1] ]
        ...
        for j in range(len_weight[1]):
            bit_value = 0; vec_weight = []; range_weight = (0, self.access_weight)
            while range_weight[0] < len_weight[0]:
                # print(f"Range {j:2d} {range_weight}")
                if range_weight[1] > len_weight[0]:
                    vec_weight += [  sram_weight_in[i][j]  for i in range(range_weight[0], len_weight[0])  ]
                else:
                    vec_weight += [  sram_weight_in[i][j]  for i in range(range_weight[0], range_weight[1])  ] 
            ...
            if j == 0 or max_out < bit_value:
                max_out = bit_value
            
            sum_out += bit_value

            # print(f"sum {j:2d}: {sum} // {range_weight}: {vec_weight}")
